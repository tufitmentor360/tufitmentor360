...
document.querySelectorAll('a[href^="#"]').forEach(a => {
  a.addEventListener('click', e => {
    ...
    if (el) {
      ...
      if (window.innerWidth < 901) nav.style.display = 'none';
    }
  });
});
...
(function(){
  ...
  // tamaño de paso = una tarjeta + gap
  const getStep = () => {
  ...
  // Convertir rueda vertical en desplazamiento horizontal (mejor UX)
...
(function(){
  ...
  function init(){
    ...
    // Mostrar sólo si no se aceptó antes
    if(!isAccepted()){
      banner.classList.add('show');        // activa display:flex !important
      banner.style.display = '';           // limpia inline por si acaso
    }else{
  ...
  // Ejecutar cuando el DOM esté listo (si script está en <head> con defer también entra)
